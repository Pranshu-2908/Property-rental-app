import { cn } from "@/lib/utils";
import React from "react";

const BgGradient = ({ className }) => {
  return (
    <div
      aria-hidden="true"
      className="pointer-events-none absolute inset-x-0 -top-40-z-10 transform-gpu overflow-hidden blur-3xl sm:-top-30"
    >
      <div
        style={{
          clipPath:
            "polygon(50% 0%, 61% 35%,90% 35%,68% 57%,79% 91%, 50% 70%, 21% 91%, 32% 57%, 2% 35%,39% 35%)",
        }}
        className={cn(
          "relative left-[calc(50%-11rem)] aspect-1155/678 w-[36,125rem] -translate-x-1/2 rotate-[30deg] bg-linear-to-br from-yellow-300 via-yellow-300 to-yellow-300 opacity-20 sm:left-[calc(50%-30rem)] sm:w-[72rem]",
          className
        )}
      />
    </div>
  );
};

export default BgGradient;
